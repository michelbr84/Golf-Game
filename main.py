...
def setup(level):  # Setup objects for the level from module courses
    ...
    if level >= 10:
        ...
        # SAFE SPAWN CHECK
        # Move ball right until it's in a valid spot (not inside wall/water/sand edge)
        safe = False
        start_x, start_y = pos
        attempts = 0
        while not safe and attempts < 100:
            collision = False
            ball_rect = pygame.Rect(start_x, start_y, 10, 10) # Approx ball size
            
            for obj in objects:
                # Check collision with hazards or walls
                # Objects format: [x, y, w, h, type]
                if obj[4] in ['wall', 'water', 'sand', 'laser', 'gap']:
            ...
            if not collision:
                ...
                start_x += 10 # Move right
                attempts += 1
        
        pos = (start_x, start_y)
        print(f"[DEBUG] Spawning ball at Level {level}: {pos} (Safe Corrected)") # Debug Spawn
        ballStationary = pos
...
def powerBar(moving=False, angle=0):
    if moving:
        ...
        pygame.draw.line(win, (255,255,255), (80, winheight -7), (int(80 + round(math.cos(angle) * 60)), int((winheight - (math.sin(angle) * 60)))), 6)
    pygame.display.update()
...
def run_game():
    global level, strokes, par, sheet, ballStationary, shootPos, powerUps, ballColor, stickyPower, superPower, mullagain, hazard, shoot, put, coins, power, loopTime, powerLock, powerAngle, neg, line
        ...
        while shoot:
            if not(overHole(ballStationary[0], ballStationary[1])):
                ...
                for i in objects:
                    if i[4] == 'coin':
                        if len(i) > 5 and i[5]:
                            ...
                            if (ball_x > coin_x - 10 and ball_x < coin_x + coin_w + 10 and 
                                ...
                                i[5] = False
                                coins += 1
                                profiles.set_coins(coins) # SAVE COINS INSTANTLY
                 ...
                 strokes = 0
                 # COINS PERSISTENCE FIX
                 coins = profiles.get_coins() # Sync after level end just in case
                 profiles.set_coins(coins)    # Ensure saved
